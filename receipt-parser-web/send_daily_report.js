...
function generateHTMLReport(data) {
    ...
    // Generate empty queries table rows
    const emptyQueriesRows = emptyQueries.slice(0, 5).map(row => `
        <tr>
            <td style="border: 1px solid #ddd; padding: 8px; max-width: 200px; word-break: break-word;">${(row.message_text || '').substring(0, 80)}${(row.message_text || '').length > 80 ? '...' : ''}</td>
    ...
    // Generate hourly stats rows
    const hourlyStatsRows = hourlyStats.slice(0, 6).map(row => `
        <tr>
...
<head>
    ...
    <title>Daily Query Logs Report - ${dateStr}</title>
    <style>
...
<body>
    <div class="header">
        <h1>📊 Daily Query Logs Analysis</h1>
        <p>Report for ${dateStr}</p>
    ...
    <div class="section">
        <h2>📈 Overall Statistics (Last 24 Hours)</h2>
        <table>
            <thead>
                <tr>
                    <th>Outcome Type</th>
    ...
    <div class="section">
        <h2>🔍 Recent Empty Result Queries</h2>
        <table>
            <thead>
                <tr>
                    <th>Message Text</th>
...
async function sendDailyReport() {
    try {
        console.log('📧 Starting daily query logs report generation...');
        
        // Run analysis silently to get data
        console.log('📊 Analyzing query logs from the last 24 hours...');
        const analysisData = await runAnalysis({ silent: true });
        ...
        // Send email
        console.log(`📤 Sending report to sma11dragon@gmail.com...`);
        const { error } = await resend.emails.send({
            from: `ReceiptAI Daily Report <${config.emailFrom}>`,
            to: 'sma11dragon@gmail.com',
            subject: `Daily Query Logs Report - ${new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`,
            html: htmlContent,
        });
